...
@NamedQueries({ @NamedQuery(name = "serverChessGameByUid", query = "SELECT x FROM ServerChessGame x where x.uid = ?1"),
		@NamedQuery(name = "getChessGamesByPlayer",
				query = "SELECT x FROM ServerChessGame x WHERE x.chessGame.whitePlayer.player.id = (SELECT p.id FROM HumanPlayer p where p.userName =?1) "
						+ "OR x.chessGame.blackPlayer.player.id = (SELECT p.id FROM HumanPlayer p where p.userName =?1)"),
		@NamedQuery(name = "getChessGameInfo",
				query = "SELECT NEW org.amc.dao.ChessGameInfo(x.uid, x.currentStatus, x.player.player.userName, x.chessGame.blackPlayer.player.userName) "
...
public abstract class ServerChessGame extends AbstractServerChessGame {
	...
	/**
	 * Adds player to the black side of the chess game Only valid if
	 * ServerChessGame is in AWAITING_PLAYER state, no exception is thrown if
	 * not in that state
	 * 
	 * @param opponent
	 *            Player
	...
	/**
	 * Sends a move to the ChessGame If successful the next player can take
	 * their turn Updates the ServerChessGame's status
	 * 
	 * @param player
	...
	/**
	 * Checks to see if the game is finished and sets it's status accordingly No
	 * check for chessGame being null
	 */
