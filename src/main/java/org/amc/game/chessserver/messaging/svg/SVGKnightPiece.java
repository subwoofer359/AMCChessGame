...
public class SVGKnightPiece extends SVGChessPiece {
	...
	public Element getChessPieceElement(String id, Location location,
		...
		float coordX = offsetXY * (location.getLetter().ordinal()), coordY = -554
				- offsetXY * (location.getNumber() - 1);

		Element g = document.createElementNS(svgNS, "g");
		g.setAttributeNS(null, "transform", "translate(" + coordX + ","
				+ coordY + ")");
		g.setAttributeNS(null, "id", id);
		g.setAttributeNS(null, "style", "fill:" + svgColour.fill);

		Element g1 = document.createElementNS(svgNS, "g");
		g1.setAttributeNS(null, "transform",
				"matrix(0.47208604,0,0,0.5491603,-79.202298,-10.14942)");

		Element[] path = new Element[10];
		
		path[0] = document.createElementNS(svgNS, "path");
		path[0].setAttributeNS(
				null,
				"d",
				"m 231.714,1837.494 c 37.531,4.161 54.642,26.149 53.007,"
						+ "94.791 h -75.885 c -0.894,-36.918 30.719,-21.425 24.182,-68.82");
		path[0].setAttributeNS(null, "style", "fill:" + svgColour.fill);

		path[1] = (Element) path[0].cloneNode(true);
		path[1].setAttributeNS(
				null,
				"d",
		...
		path[2] = (Element) path[0].cloneNode(true);
		path[2].setAttributeNS(
				null,
				"d",
				"m 190.861,1888.158 c 0,0.902 -0.732,1.635 -1.635,1.635 "
						...
						+ "0 1.635,0.732 1.635,1.634 z");

		path[3] = (Element) path[0].cloneNode(true);
		path[3].setAttributeNS(
				null,
				"d",
				"m 208.618,1856.289 c -1.354,2.345 -3.085,3.88 -3.867,3.429 "
						...
						+ "0.782,0.451 0.318,2.717 -1.035,5.063 z");

		path[4] = (Element) path[0].cloneNode(true);
		path[4].setAttributeNS(
				null,
				"d",
				"m 240.048,1838.802 -0.98,3.596 1.798,0.326 c 10.137,1.56 20.667,"
						...
						+ "-30.028,-23.392 l -1.676,-0.326 z");

		path[5] = (Element) path[0].cloneNode(true);
		path[5].setAttributeNS(null, "d",
				"m 232.819,1840.39 c 32.206,3.067 50.609,24.541 49.076,88.96 h "
						+ "-70.547 c 0,-27.608 30.673,-19.939 24.538,-64.419");
		path[5].setAttributeNS(null, "style", "stroke:" + svgColour.stroke
				+ ";stroke-linecap:round");

		path[6] = (Element) path[0].cloneNode(true);
		path[6].setAttributeNS(
				null,
				"d",
				"m 238.954,1864.931 c 1.18,8.931 -17.032,22.604 -24.538,27.607 "
						...
						+ "9.202 3.066,9.202");
		path[6].setAttributeNS(null, "style", "stroke:" + svgColour.stroke
				+ ";stroke-linecap:round;stroke-linejoin:round");

		path[7] = (Element) path[0].cloneNode(true);
		path[7].setAttributeNS(
				null,
				"d",
		...
		path[9] = document.createElement("path");
		path[9].setAttributeNS(
				null,
				"d",
